...
public struct SyncBackupPayload {
    public var schedules: [ScheduleDelta]
    public var calendarNames: [String]
    public var recurringCompletionBySchedule: [String: [TimeInterval]]

    public init(schedules: [ScheduleDelta], calendarNames: [String], recurringCompletionBySchedule: [String: [TimeInterval]]) {
        self.schedules = schedules
        self.calendarNames = calendarNames
        self.recurringCompletionBySchedule = recurringCompletionBySchedule
    }
}
...
public enum BlobConflictAction {
    case keepLocal
    case keepRemote
    case merge(Schedule)
    case askUser
...
public enum SyncConflictResolver {
    public static func localWithUnion(local: ScheduleDelta, remote: ScheduleDelta) -> ScheduleDelta {
        var merged = local
    ...
    public static func remoteWithUnion(local: ScheduleDelta, remote: ScheduleDelta) -> ScheduleDelta {
        var merged = remote
        merged.excludedOccurrenceDates = unionExcludedDates(local: local, remote: remote)
        merged.tags = unionTags(local: local, remote: remote)
        merged.syncVersion = max(local.syncVersion, remote.syncVersion)
    ...
    public static func isConflictUserVisible(local: ScheduleDelta, remote: ScheduleDelta) -> Bool {
    ...
    public static func conflictSummary(local: ScheduleDelta, remote: ScheduleDelta) -> String {
        var fields: [String] = []
    ...
    public static func resolve(local: ScheduleDelta, remote: ScheduleDelta) -> MergeOutcome {
    ...
    public static func resolveFullBlob(local: SyncBackupPayload, remote: SyncBackupPayload) -> SyncBackupPayload {
    ...
    public static func resolveBlobRestoreConflict(localSchedule: Schedule, remoteSchedule: Schedule) -> BlobConflictAction {
    ...
    private static func mergeSchedules(local: Schedule, remote: Schedule) -> Schedule {
        var merged = Schedule(
            ...
            repeatInterval: max(local.repeatInterval, remote.repeatInterval),
            repeatEndCount: max(local.repeatEndCount, remote.repeatEndCount),
            additionalReminderMinutes: Array(Set(local.additionalReminderMinutes + remote.additionalReminderMinutes)).sorted(),
            timeZoneIdentifier: local.timeZoneIdentifier ?? remote.timeZoneIdentifier
        )
